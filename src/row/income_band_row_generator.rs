use crate::row::{AbstractRowGenerator, RowGenerator, RowGeneratorResult, IncomeBandRow};
...
impl IncomeBandRowGenerator {
    ...
    fn generate_income_band_row(&mut self, row_number: i64, _session: &Session) -> Result<IncomeBandRow> {
        // Create null bit map (createNullBitMap call)
        let nulls_stream = self.abstract_generator.get_random_number_stream(&IncomeBandGeneratorColumn::IbNulls);
        ...
        let ib_lower_bound = DemographicsDistributions::get_income_band_lower_bound_at_index((row_number - 1) as usize)?;
        let ib_upper_bound = DemographicsDistributions::get_income_band_upper_bound_at_index((row_number - 1) as usize)?;
