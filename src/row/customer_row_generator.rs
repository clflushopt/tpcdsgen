...
impl CustomerRowGenerator {
    ...
    fn generate_customer_row(&mut self, row_number: i64, session: &Session) -> Result<CustomerRow> {
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CPreferredCustFlag);
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CCurrentHdemoSk);
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CCurrentCdemoSk);
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CCurrentAddrSk);
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CFirstName);
        let name_index =
        ...
        let female_name_weight =
            NamesDistributions::get_weight_for_index(name_index, FirstNamesWeights::FemaleFrequency)?;
        let salutation_weight = if female_name_weight == 0 {
            SalutationsWeights::Male
        } else {
            SalutationsWeights::Female
        };
        let stream = self.abstract_generator.get_random_number_stream(&CSalutation);
        let c_salutation =
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CEmailAddress);
        let c_email_address =
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CLastReviewDate);
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CFirstSalesDateId);
        ...
        let stream = self.abstract_generator.get_random_number_stream(&CBirthCountry);
