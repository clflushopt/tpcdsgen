...
use crate::distribution::utils::{WeightsBuilder, pick_random_index};
...
impl FipsCountyDistribution {
        ...
        for (values, weights) in parsed_lines {
            ...
            if weights.len() != Self::NUM_WEIGHT_FIELDS {
                return Err(TpcdsError::new(&format!(
                    ...
                    Self::NUM_WEIGHT_FIELDS, weights.len(), weights
                )));
            }
            ...
            zip_prefixes.push(values[4].parse::<i32>().map_err(|e| {
                TpcdsError::new(&format!("Failed to parse zip prefix '{}': {}", values[4], e))
            })?);
            gmt_offsets.push(values[5].parse::<i32>().map_err(|e| {
                TpcdsError::new(&format!("Failed to parse GMT offset '{}': {}", values[5], e))
            })?);
    ...
    pub fn pick_random_index(weights: FipsWeights, stream: &mut dyn RandomNumberStream) -> Result<usize> {
    ...
    pub fn get_state_abbreviation_at_index(index: usize) -> Result<&'static str> {
        let instance = Self::get_instance();
        instance.state_abbreviations.get(index)
            .map(|s| s.as_str())
            .ok_or_else(|| TpcdsError::new(&format!("State abbreviation index {} out of range", index)))
    }
    
    pub fn get_zip_prefix_at_index(index: usize) -> Result<i32> {
        let instance = Self::get_instance();
        instance.zip_prefixes.get(index)
            .copied()
    ...
    pub fn get_gmt_offset_at_index(index: usize) -> Result<i32> {
        let instance = Self::get_instance();
        instance.gmt_offsets.get(index)
            .copied()
