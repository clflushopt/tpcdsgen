...
use crate::distribution::utils::{WeightsBuilder, pick_random_value, pick_random_index, get_value_for_index_mod_size, get_weight_for_index};
...
impl StringValuesDistribution {
    ...
    pub fn pick_random_value(
        ...
        if value_list_index >= self.values_lists.len() {
            return Err(TpcdsError::new(&format!(
                ...
                value_list_index, self.values_lists.len() - 1
            )));
        }
        ...
        if weight_list_index >= self.weights_lists.len() {
            return Err(TpcdsError::new(&format!(
                ...
                weight_list_index, self.weights_lists.len() - 1
            )));
        }
    ...
    pub fn get_value_for_index_mod_size(&self, index: i64, value_list_index: usize) -> Result<&str> {
        if value_list_index >= self.values_lists.len() {
            return Err(TpcdsError::new(&format!(
                "Value list index {} out of range, max is {}",
                value_list_index, self.values_lists.len() - 1
    ...
    pub fn get_value_at_index(&self, value_list_index: usize, value_index: usize) -> Result<&str> {
        ...
        if value_index >= self.values_lists[value_list_index].len() {
            return Err(TpcdsError::new(&format!(
                ...
                value_index, self.values_lists[value_list_index].len() - 1
            )));
        }
