use std::fs::File;
use std::io::{BufWriter, Write};
use std::path::Path;
use tpcdsgen::config::Session;
use tpcdsgen::row::{CustomerDemographicsRowGenerator, RowGenerator, TableRow};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let session = Session::get_default_session();

    let mut generator = CustomerDemographicsRowGenerator::new();

    let output_path = Path::new("customer_demographics.dat");
    let file = File::create(output_path)?;
    let mut writer = BufWriter::new(file);

    let num_rows = session
        .get_scaling()
        .get_row_count(tpcdsgen::config::Table::CustomerDemographics);

    println!("Generating {} customer demographics rows...", num_rows);

    for row_number in 1..=num_rows {
        let result = generator.generate_row_and_child_rows(row_number, &session, None, None)?;

        generator.consume_remaining_seeds_for_row();

        let rows = result.get_rows();

        for row in rows {
            row.write_to(&mut writer, '|')?;
        }

        if row_number % 100_000 == 0 {
            println!("Progress: {} rows generated", row_number);
        }
    }

    writer.flush()?;
    println!(
        "Generated customer demographics data written to: {}",
        output_path.display()
    );
    println!("File contains {} rows", num_rows);

    Ok(())
}
