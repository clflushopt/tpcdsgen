...
use tpcdsgen::row::{PromotionRowGenerator, RowGenerator, TableRow};

fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let session = Session::get_default_session();
    let mut generator = PromotionRowGenerator::new();

    let output_path = Path::new("promotion.dat");
    ...
    println!("Generating {} promotion rows...", num_rows);
    ...
    writer.flush()?;
    println!(
        "✓ Generated promotion data written to: {}",
        output_path.display()
    );
