...
use tpcdsgen::row::{RowGenerator, TableRow, WebSiteRowGenerator};

fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let session = Session::get_default_session();
    let mut generator = WebSiteRowGenerator::new();

    let output_path = Path::new("web_site.dat");
    ...
    println!("Generating {} web_site rows...", num_rows);
    ...
    writer.flush()?;
    println!(
        "✓ Generated web_site data written to: {}",
        output_path.display()
    );
