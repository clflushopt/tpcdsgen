...
use tpcdsgen::row::{RowGenerator, TimeDimRowGenerator};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let session = Session::get_default_session();
    let mut generator = TimeDimRowGenerator::new();

    let output_path = Path::new("time_dim.dat");
    let file = File::create(output_path)?;
    let mut writer = BufWriter::new(file);

    // Get number of rows for time_dim at scale 1
    let num_rows = session
        .get_scaling()
        .get_row_count(Table::TimeDim);

    println!("Generating {} time_dim rows...", num_rows);
    ...
    writer.flush()?;
    println!(
        "Generated time_dim data written to: {}",
        output_path.display()
    );
