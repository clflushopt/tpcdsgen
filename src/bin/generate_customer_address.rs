...
//! Binary to generate customer_address table data
...
use tpcdsgen::row::{CustomerAddressRowGenerator, RowGenerator, TableRow};

fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let session = Session::get_default_session();
    let mut generator = CustomerAddressRowGenerator::new();

    let output_path = Path::new("customer_address.dat");
    ...
    let num_rows = session
        .get_scaling()
        .get_row_count(tpcdsgen::config::Table::CustomerAddress);

    println!("Generating {} customer_address rows...", num_rows);
    ...
    writer.flush()?;
    println!(
        "✓ Generated customer_address data written to: {}",
        output_path.display()
    );
