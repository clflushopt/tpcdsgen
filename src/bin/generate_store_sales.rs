...
//! Binary to generate store_sales and store_returns table data
...
use tpcdsgen::row::{RowGenerator, StoreSalesRowGenerator, TableRow};

fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let session = Session::get_default_session();
    let mut generator = StoreSalesRowGenerator::new();

    let store_sales_path = Path::new("store_sales.dat");
    let store_returns_path = Path::new("store_returns.dat");

    let store_sales_file = File::create(store_sales_path)?;
    let store_returns_file = File::create(store_returns_path)?;

    let mut store_sales_writer = Iso8859Writer::new(BufWriter::new(store_sales_file));
    let mut store_returns_writer = Iso8859Writer::new(BufWriter::new(store_returns_file));

    // Get row count (number of ORDERS) for scale 1
    let num_orders = session
        .get_scaling()
        .get_row_count(tpcdsgen::config::Table::StoreSales);

    println!("Generating store_sales from {} orders...", num_orders);

    let mut store_sales_count = 0;
    let mut store_returns_count = 0;
    ...
    while row_number <= num_orders {
        ...
        if result.should_end_row() {
            ...
            generator.consume_child_seeds(); // Also consume child (store_returns) generator seeds
            row_number += 1;
        }
    }

    store_sales_writer.flush()?;
    store_returns_writer.flush()?;

    println!(
        "Generated {} store_sales rows written to: {}",
        store_sales_count,
        store_sales_path.display()
    );
    println!(
        "Generated {} store_returns rows written to: {}",
        store_returns_count,
        store_returns_path.display()
    );
