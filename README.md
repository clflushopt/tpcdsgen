...
## Known Bugs Replicated from Java/C Implementation
...
## Why These Bugs Must Be Preserved

The TPC-DS benchmark requires exact compatibility with the reference implementation. Any deviation, even fixing obvious bugs, would produce different data and invalidate benchmark results. The data generated must be byte-for-byte identical to ensure:

1. **Benchmark Validity**: Results can be compared across different implementations
2. **Reproducibility**: The same scale factor produces identical data
3. **Certification**: Official TPC-DS results require exact compliance

These bugs have become part of the de facto standard and must be preserved in any compliant implementation.

## Verification

Both `date_dim` and `time_dim` tables have been verified to produce identical output to the Java reference implementation:

- **date_dim**: 73,049 rows with 0 differences
- **time_dim**: 86,400 rows with 0 differences

This confirms that all bugs have been successfully replicated in the Rust implementation.